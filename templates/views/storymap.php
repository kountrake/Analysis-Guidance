<div class="w-full">
    <div class="flex justify-center bg-white w-full mt-10 p-4 mb-12">
        <h1 class="text-center text-4xl font-bold underline">Story Map</h1>
    </div>

    <div class="flex flex-col w-full px-8 pr-40 bg-white">
        <p class="relative left-full top-40 ml-20">Thèmes</p><p class="relative left-full top-96 ml-20">Epics</p>
        <div class="my-4 mx-2 mb-0 w-full border-l-2 border-t-2 border-black pt-2 pl-2">
            <form method="post">
                <div class="flex flex-col justify-around w-1/6 bg-white mb-4 p-4 float-left h-48 border-grey border-solid border-2">
                    <select id="monSelect">
                        <option value="valeur1">Valeur1</option>
                        <option value="valeur1">Valeur1</option>
                        <option value="valeur1">Valeur1</option>
                    </select>
                </div>
                <div class="flex flex-col justify-around w-1/6 bg-white mb-4 p-4 float-left h-48 border-grey border-solid border-2">
                    <select id="monSelect">
                        <option value="valeur1">Valeur1</option>
                        <option value="valeur1">Valeur1</option>
                        <option value="valeur1">Valeur1</option>
                    </select>
                </div>
                <div class="flex flex-col justify-around w-1/6 bg-white mb-4 p-4 float-left h-48 border-grey border-solid border-2">
                    <select id="monSelect">
                        <option value="valeur1">Valeur1</option>
                        <option value="valeur1">Valeur1</option>
                        <option value="valeur1">Valeur1</option>
                    </select>
                </div>
                <div class="flex flex-col justify-around w-1/6 bg-white mb-4 p-4 float-left h-48 border-grey border-solid border-2">
                    <select id="monSelect">
                        <option value="valeur1">Valeur1</option>
                        <option value="valeur1">Valeur1</option>
                        <option value="valeur1">Valeur1</option>
                    </select>
                </div>
                <div class="flex flex-col justify-around w-1/6 bg-white mb-4 p-4 float-left h-48 border-grey border-solid border-2">
                    <select id="monSelect">
                        <option value="valeur1">Valeur1</option>
                        <option value="valeur1">Valeur1</option>
                        <option value="valeur1">Valeur1</option>
                    </select>
                </div>
                <div class="flex flex-col justify-around w-1/6 bg-white mb-4 p-4 float-left h-48 border-grey border-solid border-2">
                    <select id="monSelect">
                        <option value="valeur1">Valeur1</option>
                        <option value="valeur1">Valeur1</option>
                        <option value="valeur1">Valeur1</option>
                    </select>
                </div>
            </form>
        </div>

        
        
        
        

        <div class="my-4 mx-2 mb-0 mt-0 w-full border-l-2 border-black pt-2 pl-2 border-t-2">
            <form method="post">
                <div class="flex flex-col justify-around w-1/6 bg-white mb-4 p-4 float-left h-48 border-grey border-solid border-2">
                    <select id="monSelect">
                        <option value="valeur1">Valeur1</option>
                        <option value="valeur1">Valeur1</option>
                        <option value="valeur1">Valeur1</option>
                    </select>
                </div>
                
                
                
                
                
            <div class="flex flex-col justify-around w-1/6 bg-white mb-4 p-4 float-left h-48 border-grey border-solid border-2">
                    <select id="monSelect">
                        <option value="valeur1">Valeur1</option>
                        <option value="valeur1">Valeur1</option>
                        <option value="valeur1">Valeur1</option>
                    </select>
                </div><div class="flex flex-col justify-around w-1/6 bg-white mb-4 p-4 float-left h-48 border-grey border-solid border-2">
                    <select id="monSelect">
                        <option value="valeur1">Valeur1</option>
                        <option value="valeur1">Valeur1</option>
                        <option value="valeur1">Valeur1</option>
                    </select>
                </div><div class="flex flex-col justify-around w-1/6 bg-white mb-4 p-4 float-left h-48 border-grey border-solid border-2">
                    <select id="monSelect">
                        <option value="valeur1">Valeur1</option>
                        <option value="valeur1">Valeur1</option>
                        <option value="valeur1">Valeur1</option>
                    </select>
                </div><div class="flex flex-col justify-around w-1/6 bg-white mb-4 p-4 float-left h-48 border-grey border-solid border-2">
                    <select id="monSelect">
                        <option value="valeur1">Valeur1</option>
                        <option value="valeur1">Valeur1</option>
                        <option value="valeur1">Valeur1</option>
                    </select>
                </div><div class="flex flex-col justify-around w-1/6 bg-white mb-4 p-4 float-left h-48 border-grey border-solid border-2">
                    <select id="monSelect">
                        <option value="valeur1">Valeur1</option>
                        <option value="valeur1">Valeur1</option>
                        <option value="valeur1">Valeur1</option>
                    </select>
                </div></form>
        </div><div class="my-4 mx-2 mt-0 mb-0 w-full border-l-2 border-black border-t-2">
            <p class="relative left-full top-56 ml-20">V1</p><form method="post">
                
                
                
            <div class="flex flex-col justify-around w-1/6 bg-white mb-4 p-4 float-left h-48">
                        
                        <textarea class="border rounded w-full py-2 px-3 text-grey-darker" id="usname" maxlength="300" rows="9" cols="33"></textarea>
                    </div><div class="flex flex-col justify-around w-1/6 bg-white mb-4 p-4 float-left h-48">
                        
                        <textarea class="border rounded w-full py-2 px-3 text-grey-darker" id="usname" maxlength="300" rows="9" cols="33"></textarea>
                    </div><div class="flex flex-col justify-around w-1/6 bg-white mb-4 p-4 float-left h-48">
                        
                        <textarea class="border rounded w-full py-2 px-3 text-grey-darker" id="usname" maxlength="300" rows="9" cols="33"></textarea>
                    </div><div class="flex flex-col justify-around w-1/6 bg-white mb-4 p-4 float-left h-48">
                        
                        <textarea class="border rounded w-full py-2 px-3 text-grey-darker" id="usname" maxlength="300" rows="9" cols="33"></textarea>
                    </div><div class="flex flex-col justify-around w-1/6 bg-white mb-4 p-4 float-left h-48">
                        
                        <textarea class="border rounded w-full py-2 px-3 text-grey-darker" id="usname" maxlength="300" rows="9" cols="33"></textarea>
                    </div><div class="flex flex-col justify-around w-1/6 bg-white mb-4 p-4 float-left h-48">
                        
                        <textarea class="border rounded w-full py-2 px-3 text-grey-darker" id="usname" maxlength="300" rows="9" cols="33"></textarea>
                    </div></form>
        </div><div class="my-4 mx-2 mt-0 mb-12 w-full border-l-2 border-black">
            <form method="post">
                
                
                
            <div class="flex flex-col justify-around w-1/6 bg-white mb-4 p-4 float-left h-48">
                        
                        <textarea class="border rounded w-full py-2 px-3 text-grey-darker" id="usname" maxlength="300" rows="9" cols="33"></textarea>
                    </div><div class="flex flex-col justify-around w-1/6 bg-white mb-4 p-4 float-left h-48">
                        
                        <textarea class="border rounded w-full py-2 px-3 text-grey-darker" id="usname" maxlength="300" rows="9" cols="33"></textarea>
                    </div><div class="flex flex-col justify-around w-1/6 bg-white mb-4 p-4 float-left h-48">
                        
                        <textarea class="border rounded w-full py-2 px-3 text-grey-darker" id="usname" maxlength="300" rows="9" cols="33"></textarea>
                    </div><div class="flex flex-col justify-around w-1/6 bg-white mb-4 p-4 float-left h-48">
                        
                        <textarea class="border rounded w-full py-2 px-3 text-grey-darker" id="usname" maxlength="300" rows="9" cols="33"></textarea>
                    </div><div class="flex flex-col justify-around w-1/6 bg-white mb-4 p-4 float-left h-48">
                        
                        <textarea class="border rounded w-full py-2 px-3 text-grey-darker" id="usname" maxlength="300" rows="9" cols="33"></textarea>
                    </div><div class="flex flex-col justify-around w-1/6 bg-white mb-4 p-4 float-left h-48">
                        
                        <textarea class="border rounded w-full py-2 px-3 text-grey-darker" id="usname" maxlength="300" rows="9" cols="33"></textarea>
                    </div></form>
        </div><div class="flex flew-row justify-around mb-4">
            <a href="/download" class="bg-blue-700 rounded border-2 border-blue-800 p-2 text-white text-semi-bold hover:underline hover:bg-blue-600">
                Télécharger
            </a>
            <a href="/userstory" class="bg-blue-700 rounded border-2 border-blue-800 p-2 text-white text-semi-bold hover:underline hover:bg-blue-600">
                Suivant
            </a>
        </div>

    </div>
</div>